...
> {
  ...
  const customers =
    await backendClient.fetch<Array<{ email: string; customerName: string }>>(
      query,
    )
  return customers ?? []
}
  ...
  try {
    if (emailType === 'followup') {
      ...
      for (const customer of customers) {
        try {
          const personalizedHtml = applyVariables(html, {
    ...
    if (audience === 'workshop-attendees') {
      ...
      for (const attendee of attendees) {
        try {
          const personalizedHtml = applyVariables(withUnsubscribeFooter(html), {
